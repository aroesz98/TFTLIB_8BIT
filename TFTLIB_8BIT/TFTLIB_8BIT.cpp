...
{
    if ((x0 > x1) || (x1 > _width) || (y0 > y1) || (y1 > _height))
        return;
...
{
    if (x < 0 || x > _width || y < 0 || y > _height || x + w > _width)
    {
        return;
    }
...
{
    if (x < 0 || x > _width || y < 0 || y > _height || y + h > _height)
    {
        return;
    }
...
{
    if (x0 < 0 || y0 < 0 || x0 > _width || y0 > _height || x0 + r > _width || y0 + r > _height)
    {
        return;
    }
...
{
    if (x0 < 0 || x0 > _width || y0 < 0 || y0 > _height || x1 < 0 || x1 > _width || y1 < 0 || y1 > _height)
    {
        return;
    }
...
{
    if (x < 0 || y < 0 || w < 0 || h < 0 || x > _width || y > _height || x + w > _width || y + h > _height)
    {
        return;
    }
...
{
    if (x0 - r < 0 || x0 + r > _width || y0 - r < 0 || y0 + r > _height)
    {
        return;
    }
...
{
    if (x - r < 0 || x + r > _width || y - r < 0 || y + r > _height)
    {
        return;
    }
    ...
    { // New font
        while (*string)
        {
            ...
            if ((uniCode >= read_word(&gfxFont->first)) && (uniCode <= read_word(&gfxFont->last)))
            {
                ...
                if (*string || isDigits)
                {
                    str_width += read_byte(&glyph->xAdvance);
                }
                // Else use the offset plus width since this can be bigger than xAdvance
                else
                {
                    str_width += ((int8_t)read_byte(&glyph->xOffset) + read_byte(&glyph->width));
                }
            }
        }
    }
...
size_t TFTLIB_8BIT::write(uint8_t utf8)
{
    ...
    {
        if (uniCode > read_word(&gfxFont->last))
        {
            return 1;
        }
        if (uniCode < read_word(&gfxFont->first))
        {
            return 1;
        }
...
int16_t TFTLIB_8BIT::drawFloat(float floatNumber, uint8_t dp, int32_t poX, int32_t poY)
{
    ...
    float rounding = 0.5f; // Round up down delta
    ...
    { // while (i < dp) for no limit but array size must be increased
        i++;
        floatNumber *= 10.0f;  // for the next decimal
